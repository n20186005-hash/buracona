  ...
  items: [
    {
      ...
      img: '/gallery/buracona-blue-eye-cave-9.jpg',
    },
    {
      icon: '🏊', num: '02',
...
export const sources = {
  ...
  weatherNote: { zh: `数据来源：公开气象与海洋观测服务`, en: `Source: public weather & marine services`, pt: `Fonte: serviços públicos de meteorologia e mar` } as T,
};
...
// ── Visitor Services & Facilities (type-neutral, merchant-agnostic) ──
export const services = {
  sectionNum: { zh: `访客服务与设施`, en: `Visitor Services`, pt: `Serviços ao Visitante` } as T,
  heading: { zh: `出发前了解<em class="text-[color:var(--color-emerald)]">这些设施</em>`, en: `Facilities to know<br/><em class="text-[color:var(--color-emerald)]"> before you arrive</em>`, pt: `Serviços a conhecer<br/><em class="text-[color:var(--color-emerald)]"> antes de chegar</em>` } as T,
  subtitle: { zh: `Buracona 地处荒野、补给有限。以下按类型概述常见设施与注意事项（不推荐任何具体商户），请按需提前准备。`, en: `Buracona is remote with limited supply. The notes below are type-based and merchant-neutral — plan ahead accordingly.`, pt: `A Buracona é remota e com pouco apoio. As notas abaixo são por tipo e neutras — planeie com antecedência.` } as T,
  cards: [
    { icon: '🚻', label: { zh: `卫生间`, en: `Restrooms`, pt: `Sanitas` } as T, text: { zh: `景区入口由社区协会管理，设有简易卫生间；条件较基础，建议自备纸巾与湿巾。`, en: `Basic restrooms at the community-managed entrance — simple facilities, bring your own tissue and wipes.`, pt: `Sanitas simples à entrada gerida pela comunidade — básicas, leve o seu papel e toalhitas.` } as T },
    { icon: '🅿️', label: { zh: `停车`, en: `Parking`, pt: `Estacionamento` } as T, text: { zh: `入口附近为非铺装土路停车区，无正规车位与标线。旺季车位紧张，请规范停车、勿阻塞通道。`, en: `Unpaved parking near the entrance, no marked bays. Fills up in peak season — park tidily and don't block access.`, pt: `Estacionamento de terra junto à entrada, sem lugares marcados. Enche na época alta — estacione bem.` } as T },
    { icon: '🍽️', label: { zh: `餐饮`, en: `Food & drink`, pt: `Comida e bebida` } as T, text: { zh: `景区内基本无餐厅，仅在入口有小摊贩售卖饮品与简易零食。建议从 Santa Maria 带足饮水与干粮。`, en: `Almost no dining inside — only small vendors at the entrance sell drinks and snacks. Bring water and food from Santa Maria.`, pt: `Quase sem restauração no local — só vendedores à entrada com bebidas e snacks. Leve comida de Santa Maria.` } as T },
    { icon: '🛏️', label: { zh: `住宿`, en: `Lodging`, pt: `Alojamento` } as T, text: { zh: `岛上住宿集中在 Santa Maria（度假酒店、民宿与公寓）；Buracona 周边无住宿。建议以 Santa Maria 为基地当日往返。`, en: `Lodging clusters in Santa Maria (resorts, guesthouses, apartments); nothing near Buracona. Base yourself in Santa Maria and do a day trip.`, pt: `O alojamento concentra-se em Santa Maria (resorts, pensões, apartamentos); nada perto da Buracona. Baseie-se em Santa Maria e faça um dia.` } as T },
    { icon: '🛒', label: { zh: `商超 / 便利店`, en: `Shops & groceries`, pt: `Lojas e mercearias` } as T, text: { zh: `补给定点在 Santa Maria 与 Espargos 的超市与便利店；景区无商超，请出发前集中采购。`, en: `Groceries are in Santa Maria and Espargos supermarkets and minimarkets; none at the site — stock up before you leave town.`, pt: `Compras em supermercados e lojas de Santa Maria e Espargos; nada no local — abasteça antes de partir.` } as T },
    { icon: '⛽', label: { zh: `加油 / 充电`, en: `Fuel & charging`, pt: `Combustível e carregamento` } as T, text: { zh: `全岛加油站集中在 Santa Maria 与 Espargos；电动车公共充电桩极少。自驾请满油或满电出发。`, en: `Petrol stations sit in Santa Maria and Espargos; public EV charging is scarce. Self-drivers should leave with a full tank or battery.`, pt: `Postos de combustível em Santa Maria e Espargos; carregamento de VE é raro. Saia com o depósito ou bateria cheios.` } as T },
    { icon: '💵', label: { zh: `现金 / ATM`, en: `Cash & ATM`, pt: `Dinheiro e ATM` } as T, text: { zh: `社区协会门票多收现金（欧元或当地埃斯库多），附近无 ATM。请在 Santa Maria 或 Espargos 取现后再前往。`, en: `The community entry fee is mostly cash (euros or escudos) and there is no ATM nearby. Withdraw in Santa Maria or Espargos first.`, pt: `A taxa comunitária é sobretudo em dinheiro (euros ou escudos) e não há ATM perto. Levante em Santa Maria ou Espargos.` } as T },
    { icon: '📶', label: { zh: `信号 / 网络`, en: `Signal & Wi-Fi`, pt: `Sinal e Wi-Fi` } as T, text: { zh: `移动信号覆盖一般，景区内可能较弱；部分营业点提供 Wi-Fi，但别指望稳定连接。`, en: `Mobile coverage is patchy and can drop at the site; some vendors offer Wi-Fi, but don't rely on it.`, pt: `Cobertura móvel fraca e pode falhar no local; alguns vendedores têm Wi-Fi, mas não conte com ele.` } as T },
  ],
};

// ── Suggested Itineraries (by traveller type + half/full day) ──
export const itineraries = {
  sectionNum: { zh: `游览路线推荐`, en: `Suggested Itineraries`, pt: `Itinerários Sugeridos` } as T,
  heading: { zh: `按你的节奏<br/><em class="text-[color:var(--color-emerald)]">安排这一天</em>`, en: `Plan your day<br/><em class="text-[color:var(--color-emerald)]"> to your pace</em>`, pt: `Planeie o seu dia<br/><em class="text-[color:var(--color-emerald)]"> ao seu ritmo</em>` } as T,
  subtitle: { zh: `无论亲子、摄影还是希望轻松慢游，这里都给出可参考的行程骨架。请结合天气、潮汐与日出日落时间灵活调整。`, en: `Whether with kids, for photography, or an easy pace — here is a flexible skeleton. Adjust to weather, tides and light.`, pt: `Com crianças, para fotografia ou passeio tranquilo — um esqueleto flexível. Ajuste a tempo, marés e luz.` } as T,
  peopleTitle: { zh: `按人群定制`, en: `By traveller type`, pt: `Por tipo de visitante` } as T,
  generalTitle: { zh: `通用路线（半日 / 全日）`, en: `Classic routes (half / full day)`, pt: `Rotas clássicas (meio / dia inteiro)` } as T,
  people: [
    { icon: '👨‍👩‍👧', title: { zh: `亲子家庭`, en: `Families`, pt: `Famílias` } as T, text: { zh: `以"蓝眼观景 + 火山岩潮池戏水 + 落日之窗"为轻松半日：避开正午暴晒，安排 10:30 前或 15:00 后到访；潮池仅限浅水区、请全程看护儿童并穿防滑鞋。`, en: `An easy half-day: Blue Eye viewpoint + volcanic pool splash + sunset window. Avoid midday sun — arrive before 10:30 or after 15:00; keep kids in the shallow pool area, supervised, in non-slip shoes.`, pt: `Meio-dia fácil: miradouro do Olho Azul + poça vulcânica + janela do sol. Evite o sol do meio-dia — chegue antes das 10:30 ou depois das 15:00; crianças na poça rasa, vigiadas e com calçado antiderrapante.` } as T },
    { icon: '📷', title: { zh: `摄影 / 自然`, en: `Photography & nature`, pt: `Fotografia e natureza` } as T, text: { zh: `正午 11:00–13:00 是蓝眼蓝光最浓的窗口；黄昏留到落日之窗拍金红海面；清晨或阴天光线柔和，适合拍摄玄武岩地貌。建议带偏振镜 / ND 与三脚架。`, en: `Blue Eye peaks 11:00–13:00; stay for the gold-red sunset window at dusk; soft morning or overcast light is best for the basalt. Bring a polariser/ND and a tripod.`, pt: `O Olho Azul é mais forte 11:00–13:00; fique para a janela do sol ao entardecer; luz suave de manhã ou nublado é melhor para o basalto. Leve polarizador/ND e tripé.` } as T },
    { icon: '🧓', title: { zh: `低体力 / 无障碍`, en: `Low-effort / accessible`, pt: `Pouco esforço / acessível` } as T, text: { zh: `入口至主景有向导陪同的平缓路段，但玄武岩地面不平、无固定无障碍设施。长者或行动不便者可由洞口平台远观蓝眼，不必下到潮池；途中多休息、带足饮水。`, en: `The entrance to the main view has a guided, gentle stretch, but the basalt ground is uneven with no fixed accessibility features. Seniors or limited-mobility visitors can view the Blue Eye from the rim platform without descending to the pool; rest often and bring water.`, pt: `Da entrada à vista principal há um troço guiado e suave, mas o basalto é irregular e sem acessibilidade fixa. Idosos ou com mobilidade reduzida podem ver o Olho Azul da plataforma da borda sem descer à poça; descanse e leve água.` } as T },
  ],
  half: {
    title: { zh: `半日精华`, en: `Half-day highlight`, pt: `Meio-dia essencial` } as T,
    meta: { zh: `约 3–4 小时`, en: `~3–4 hours`, pt: `~3–4 horas` } as T,
    steps: { zh: `Santa Maria 出发 → Buracona 蓝眼（正午最佳）→ 火山岩潮池 → 落日之窗 → 返回`, en: `From Santa Maria → Buracona Blue Eye (best at noon) → volcanic pool → sunset window → return`, pt: `De Santa Maria → Olho Azul da Buracona (melhor ao meio-dia) → poça vulcânica → janela do sol → regresso` } as T,
  },
  full: {
    title: { zh: `全日环岛`, en: `Full-day island loop`, pt: `Volta à ilha (dia inteiro)` } as T,
    meta: { zh: `约 6–8 小时`, en: `~6–8 hours`, pt: `~6–8 horas` } as T,
    steps: { zh: `Santa Maria → Buracona（蓝眼 + 潮池 + 落日之窗）→ 经 Palmeira 渔村午餐 → Pedra de Lume 盐湖漂浮 → Baía da Murdeira 观鸟 → 返回`, en: `Santa Maria → Buracona (Blue Eye + pool + sunset window) → lunch at Palmeira fishing village → float at Pedra de Lume salt lake → birdwatch at Baía da Murdeira → return`, pt: `Santa Maria → Buracona (Olho Azul + poça + janela do sol) → almoço na vila piscatória de Palmeira → flutuar nas salinas de Pedra de Lume → aves na Baía da Murdeira → regresso` } as T,
  },
};

// ── Climate · Sea · Seasons (synthesis + seasonal strategy table) ──
export const climateMarine = {
  sectionNum: { zh: `气候 · 海洋 · 季节`, en: `Climate · Sea · Seasons`, pt: `Clima · Mar · Estações` } as T,
  heading: { zh: `什么时候来<br/><em class="text-[color:var(--color-emerald)]">最合适</em>`, en: `When is<br/><em class="text-[color:var(--color-emerald)]"> the best time</em>`, pt: `Qual a<br/><em class="text-[color:var(--color-emerald)]"> melhor altura</em>` } as T,
  subtitle: { zh: `萨尔岛属干旱热带气候，全年晴日多、降水极少。以下综合长期气候态（多年平均）与区域海洋观测，帮您判断不同季节的天气、海况与生态亮点。`, en: `Sal is an arid tropical island — mostly sunny with very little rain. The notes below combine long-term climatological means with regional marine observations to read each season's weather, sea and wildlife.`, pt: `O Sal é uma ilha tropical árida — sobretudo soalheiro com pouca chuva. As notas juntam médias climatológicas de longo prazo e observações marinhas regionais para ler o tempo, o mar e a vida de cada estação.` } as T,
  synthesisTitle: { zh: `全年气候 · 水质 · 生态综合分析`, en: `Year-round climate, water & ecology`, pt: `Clima, água e ecologia ao ano` } as T,
  synthesis: { zh: `萨尔岛全年温暖（年均约 25°C），最热为 7–9 月（26–29°C），最凉为 12–2 月（约 21–25°C）。降水极少（年均 50–150 mm），多集中在 8–10 月与 2–3 月的短暂阵雨。东北信风在 12–3 月最强，是风帆与风筝冲浪的旺季。海水温度终年 22–26°C，盐度高、通透度好，是浮潜与观鱼的好条件。生态上：海龟 6–10 月筑巢，火烈鸟在雨季积水时聚集于 Baía da Murdeira，鲸类在 3–5 月迁徙途中偶有现身。`, en: `Sal stays warm all year (annual mean ≈25°C): hottest in Jul–Sep (26–29°C), coolest in Dec–Feb (≈21–25°C). Rain is scarce (≈50–150 mm/yr), mostly brief showers in Aug–Oct and Feb–Mar. The NE trade winds peak in Dec–Mar, the kitesurf season. Sea temperature is 22–26°C year-round with high salinity and good clarity — ideal for snorkelling. Ecologically, turtles nest Jun–Oct, flamingos gather at Baía da Murdeira when the lagoon fills in the wet season, and whales may appear on their Mar–May migration.`, pt: `O Sal mantém-se quente o ano todo (média ≈25°C): mais quente em jul–set (26–29°C), mais fresco em dez–fev (≈21–25°C). Chuva rara (≈50–150 mm/ano), sobretudo aguaceiros em ago–out e fev–mar. Os alísios de NE são mais fortes em dez–mar, época de kitesurf. A água do mar fica a 22–26°C todo o ano, com salinidade alta e boa claridade — ideal para snorkel. Ecologicamente, as tartarugas nidificam jun–out, os flamingos juntam-se na Baía da Murdeira quando a lagoa enche, e as baleias podem surgir na migração de mar–mai.` } as T,
  tableTitle: { zh: `季度游览策略表`, en: `Seasonal strategy table`, pt: `Tabela de estratégia sazonal` } as T,
  colPeriod: { zh: `时段`, en: `Period`, pt: `Período` } as T,
  colWeather: { zh: `天气与气温`, en: `Weather & temp`, pt: `Tempo e temp.` } as T,
  colSea: { zh: `风与海况`, en: `Wind & sea`, pt: `Vento e mar` } as T,
  colPlan: { zh: `蓝眼与游玩建议`, en: `Blue Eye & tips`, pt: `Olho Azul e dicas` } as T,
  colWild: { zh: `生态亮点`, en: `Wildlife highlight`, pt: `Destaque ecológico` } as T,
  rows: [
    { period: { zh: `干季 · 旺季`, en: `Dry · peak`, pt: `Seca · alta` } as T, meta: { zh: `11月–次年4月`, en: `Nov–Apr`, pt: `nov–abr` } as T, weather: { zh: `晴朗少雨，23–27°C，最稳定的出行期。`, en: `Sunny and dry, 23–27°C — the most stable window.`, pt: `Soalheiro e seco, 23–27°C — a janela mais estável.` } as T, sea: { zh: `微风，海况平稳，适合下水与观景。`, en: `Light wind, calm seas — good for swimming and views.`, pt: `Vento fraco, mar calmo — bom para banhos e vista.` } as T, plan: { zh: `正午蓝眼最佳，全天皆宜。`, en: `Blue Eye peaks at noon; good any time.`, pt: `Olho Azul ao meio-dia; bom a qualquer hora.` } as T, wild: { zh: `Baía da Murdeira 火烈鸟聚集（雨季积水）。`, en: `Flamingos gather at Baía da Murdeira (lagoon filled).`, pt: `Flamingos na Baía da Murdeira (lagoa cheia).` } as T },
    { period: { zh: `风季`, en: `Windy season`, pt: `Época dos ventos` } as T, meta: { zh: `12月–3月`, en: `Dec–Mar`, pt: `dez–mar` } as T, weather: { zh: `凉爽 21–25°C，东北信风最强。`, en: `Cool 21–25°C, NE trade winds strongest.`, pt: `Fresco 21–25°C, alísios de NE mais fortes.` } as T, sea: { zh: `风大、海面易起浪，海边风感明显。`, en: `Windy, choppy seas, exposed coastline.`, pt: `Ventoso, mar agitado, costa exposta.` } as T, plan: { zh: `蓝眼仍可观，注意防风防沙；避开岩台风口。`, en: `Blue Eye still fine — mind wind and sand; avoid exposed ledges.`, pt: `Olho Azul continua bom — cuidado com vento e areia; evite bordas.` } as T, wild: { zh: `风帆 / 风筝冲浪旺季，海鸟活跃。`, en: `Kitesurf season; seabirds active.`, pt: `Época de kitesurf; aves marinhas ativas.` } as T },
    { period: { zh: `热季`, en: `Hot season`, pt: `Época quente` } as T, meta: { zh: `7月–9月`, en: `Jul–Sep`, pt: `jul–set` } as T, weather: { zh: `最热 26–29°C，湿度上升，偶有热带扰动 / 降雨。`, en: `Hottest 26–29°C, humid, occasional tropical disturbance/rain.`, pt: `Mais quente 26–29°C, húmido, às vezes perturbação/chuva tropical.` } as T, sea: { zh: `午后闷热，海上偶有涌浪。`, en: `Muggy afternoons, occasional swell.`, pt: `Tardes abafadas, ondulação ocasional.` } as T, plan: { zh: `清晨或傍晚出游，正午短暂看蓝眼即可。`, en: `Go early or late; a short noon visit for the Blue Eye.`, pt: `Vá cedo ou tarde; visita curta ao meio-dia para o Olho Azul.` } as T, wild: { zh: `海龟筑巢季（6–10月）。`, en: `Turtle nesting season (Jun–Oct).`, pt: `Época de nidificação das tartarugas (jun–out).` } as T },
    { period: { zh: `过渡 / 小雨季`, en: `Transition / light rain`, pt: `Transição / chuva leve` } as T, meta: { zh: `8月–10月、2月–3月`, en: `Aug–Oct, Feb–Mar`, pt: `ago–out, fev–mar` } as T, weather: { zh: `降雨概率略升，多为短时阵雨。`, en: `Slightly higher rain chance, mostly brief showers.`, pt: `Probabilidade de chuva um pouco maior, sobretudo aguaceiros.` } as T, sea: { zh: `天气多变，雨后海面通透。`, en: `Changeable; seas clear after rain.`, pt: `Variável; mar limpo após chuva.` } as T, plan: { zh: `雨后空气通透、拍照出片；避开降雨时段。`, en: `Clear air after rain makes great photos; avoid wet spells.`, pt: `Ar limpo após chuva dá boas fotos; evite a chuva.` } as T, wild: { zh: `鲸类迁徙（3–5月）偶有现身。`, en: `Whale migration (Mar–May), sometimes seen.`, pt: `Migração das baleias (mar–mai), por vezes vistas.` } as T },
  ],
  basis: { zh: `说明：以上基于萨尔岛长期气候态（多年平均）与区域海洋观测整理，仅供参考；出行前请以佛得角官方气象与旅游渠道发布的最新信息为准。`, en: `Note: based on Sal's long-term climatological means (multi-year normals) and regional marine observations, for reference only — verify the latest official Cape Verde weather and tourism advisories before travelling.`, pt: `Nota: baseado nas médias climatológicas de longo prazo do Sal (normais plurianuais) e observações marinhas regionais, só para referência — verifique os avisos oficiais de Cabo Verde antes de viajar.` } as T,
};
